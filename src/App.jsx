...
function OFPBriefingView({ flight, currentUser, onSign, onUpdateFlight, onEditFlight, onDeleteFlight, appSettings, airports }) {
  ...
  const mockWeather = (icao, type = '') => {
    if (!icao) return null;
    if (type.includes('DEP')) {
      return {
        metar: `METAR ${icao} 280700Z 06012KT 9999 FEW020 BKN050 24/18 Q1015 NOSIG=`,
        taf: `TAF ${icao} 280500Z 2806/0112 05010KT 9999 SCT025 BKN040=`
      };
    }
    if (type.includes('ALTN')) {
      return {
        metar: `METAR ${icao} 280700Z 18015G25KT 5000 -RA BR FEW015 BKN030 18/16 Q1008 NOSIG=`,
        taf: `TAF ${icao} 280500Z 2806/0112 19015KT 6000 -RA BKN020 PROB30 TEMPO 2808/2812 3000 TSRA=`
      };
    }
    return {
  ...
  const generateAIBriefing = async () => {
    ...
    const notamsPrompt = aptPromptItems.map(item => {
        const nList = getNotamsForIcao(item.icao, airports);
        const joinedNotams = nList.map(n => n.replace(/\n/g, ' ')).join('\n  - ');
        return `[${item.type}] ${item.icao}:\n  - ${joinedNotams}`;
    }).join('\n');

    const weatherPrompt = aptPromptItems.map(item => {
        const wx = mockWeather(item.icao, item.type);
        return `[${item.type}] ${item.icao}:\n  - ${wx.metar}\n  - ${wx.taf}`;
    }).join('\n');
...
2. 內容需包含：油量充裕度評估、機況限制評估、航路潛在風險、起降場/備降場注意事項(含天氣與NOTAM)。
...
4. 【氣象評估與 NOTAM】：請根據 METAR 與 TAF 評估各機場能見度、雲冪高、風速風向等天氣風險；並留意 NOTAM 內跑道關閉或設施限制等對飛安或油量的影響。
...
`;
      ...
      console.log('🤖 [AI 分析] 發送給 Gemini 的提示詞 (Prompt):', prompt);
      ...
      <div className="bg-[#1e293b] border border-slate-700 rounded-lg p-4 sm:p-6 md:p-8 shadow-2xl font-mono text-xs sm:text-sm text-slate-300 print:bg-white print:text-black print:border-none print:shadow-none print:p-0 w-full overflow-hidden">
        ...
        <div className="mt-8">
           ...
           <div className="bg-slate-900 print:bg-transparent print:p-0 print:border-none p-4 rounded-md border border-slate-700 text-xs sm:text-sm space-y-4">
              {uniqueAptItems.map((item, idx) => {
                 const wx = mockWeather(item.icao, item.type.replace(' ', ''));
                 const isGreen = item.type === 'DEP' || item.type === 'ARR';
                 return (
                   <div key={`wx-${idx}`} className={`${idx > 0 ? "border-t border-slate-700 print:border-gray-300 pt-3 " : ""}break-words`}>
                     <span className="inline-block bg-slate-800 print:bg-gray-200 text-slate-300 print:text-gray-700 px-2 py-0.5 rounded text-xs mr-2 font-bold">[{item.type}]</span>
                     <span className={`${isGreen ? 'text-green-400' : 'text-yellow-400'} print:text-black font-bold text-sm sm:text-base`}>{item.icao}</span>
                     <p className="text-slate-400 print:text-black mt-1.5">{wx.metar}</p>
                     <p className="text-slate-400 print:text-black mt-0.5">{wx.taf}</p>
                   </div>
                 );
              })}
           </div>
